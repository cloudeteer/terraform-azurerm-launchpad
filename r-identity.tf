locals {
  user_assigned_identity_name = var.name_overrides.user_assigned_identity != null ? var.name_overrides.user_assigned_identity : join("-", compact(["id", var.name, "prd", local.location_short[var.location], var.name_suffix]))
}
